...
| 7 | HUD Charges (NORMAL mode) | Title insurance, CPL fee, endorsements, legal/settlement, recording, inspection, appraisal, insurance, transfer tax, other 3rd party; **plus PA BEC fields**: mechanics lien insurance, survey, domestic/patriot act lien search, pest/radon/water/sewer inspections (prepaid), county/municipal/school tax prorations, lender insurance & tax escrows, prepaid interest, HOA proration & capital contribution, lienable utilities, home warranty | ✅ COMPLETE |
| 8 | HUD Charges (HIDEOUT mode) | Walker fees (doc prep, overnight, wire, attorney, notary, settlement), Hideout transfer fee (manual entry), prorated dues/taxes (city, school, sewer/water), survey fee, domestic/patriot searches; **sell-side**: legal/settlement fees + Hideout transfer fee in exit costs | ✅ COMPLETE |
...
|------|---------|-------------|----------------|
| 2026-03-06 | Third-Party Closing Cost Estimator — Apply to Fields | Enhanced estimator: clicking MIN/AVG/MAX "Apply" button distributes that total across 9 real closing cost fields (Legal & Settlement 20%, Insurance 15%, Other 3rd Party 21%, Survey 10%, Prepaid Interest 12%, Recording 8%, Home Warranty 8%, Domestic Lien Search 4%, Patriot Act Search 2%). Applied card turns green with "✓ Applied" state; breakdown panel shows each field + amount distributed; note clarifies title insurance & transfer tax are handled separately as % fields. Range change resets Applied state. Added `useState` import to InputSections.tsx. Build: 764 modules clean. | `components/InputSections.tsx` |
| 2026-03-06 | Third-Party Closing Cost Estimator | Added a new "Third-Party Closing Cost Estimator" section between HUD Charges and Exit Strategy. Features: (1) 3-button range selector — LOW (0.5–1.0%), MID (1.0–2.0%, default), HIGH (2.0–4.0%) with blue highlight on selected; (2) Live result cards showing Minimum, Average (highlighted in blue), and Maximum dollar amounts calculated from purchase price × range percentages; (3) Graceful empty state when purchase price is $0; (4) `thirdPartyClosingCostRange` field added to `LoanInputs` / `DEFAULT_INPUTS` so selection persists with saved deals. No existing functionality changed. Build: 764 modules clean. | `types.ts`, `components/InputSections.tsx` |
| 2026-03-06 | NORMAL Mode Closing Cost Expansion (PA BEC Full Coverage) | Added 18 new closing cost fields to NORMAL mode based on Pennsylvania Association of Realtors Buyer's Estimated Cost Sheet (BEC). **Closing fees** (flow into totalThirdPartyFees → totalClosingCosts): Mechanics Lien Insurance (1D), Survey Fee (1H), Domestic Lien Search (1I), Patriot Act Search (1J), Lender Insurance Escrow (3B), County Tax Proration (4B), Municipal Tax Proration (4C), School Tax Proration (4A), Lender Tax Escrow (4D), HOA Proration (4E), HOA Capital Contribution (4F), Lienable Utilities (4G), Home Warranty (7), Prepaid Interest to end of month (6G). **Prepaid fees** (flow into prepaidCosts): Pest/Wood Infestation (5B), Radon (5C), Water Test (5D), Sewer (5E). All default to $0. UI organized into 5 labeled subsections in HUD Charges section. LoanEstimateCard shows all new line items (hidden when $0) in both Third Party Fees and Cash Required breakdown. Build: 764 modules clean. | `types.ts`, `utils/calculations.ts`, `components/InputSections.tsx`, `components/LoanEstimateCard.tsx` |
| 2026-02-24 | Fix: Root cause — Hideout fees always calculating in NORMAL mode | `DEFAULT_INPUTS.appVersion` was `'HIDEOUT'` in `types.ts`, so `calculateLoan(inputs)` always received HIDEOUT regardless of the UI version selector — causing Walker Fees, Dues, City/Town Taxes, School Tax, Sewer & Water to always have real values. Fixed: (1) `DEFAULT_INPUTS.appVersion` → `'NORMAL'`; (2) `handleVersionChange` now also sets `inputs.appVersion` so the calculation engine receives the correct version on every switch; (3) Preference-load that forces NORMAL now also updates `inputs.appVersion`. Build: 764 modules clean. | `types.ts`, `App-refactored.tsx` |
| 2026-02-24 | Fix: 404 + Walker rows in NORMAL mode | Created `index.css` to resolve 404 (`<link rel="stylesheet" href="/index.css">` was in index.html but file didn't exist). Wrapped Walker Fees row in `ClosingProfitCard.tsx` with `{results.totalWalkerFees > 0 && (...)}`. Wrapped Walker & Walker Fees row in `ProfitTable.tsx` with `{results.totalWalkerFees > 0 && (...)}`. Fixed `ReportMode.tsx` default `appVersion` from `'HIDEOUT'` to `'NORMAL'`. Build: 764 modules clean. | `index.css` (new), `components/ClosingProfitCard.tsx`, `components/ProfitTable.tsx`, `ReportMode.tsx` |
| 2026-02-24 | NORMAL mode hides all Hideout-specific UI fields | In NORMAL mode, InputSections now hides: Walker Doc/Overnight/Wire fees, Hideout Transfer + Dues, City/Town Taxes (roamingwoodAnnual), School Tax (schoolTaxAnnual), Sewer & Water. All were already zeroed in calculations — now also hidden from the UI. Switching back to HIDEOUT restores all fields with their stored values. Fixed InputSections default `appVersion` from `'HIDEOUT'` to `'NORMAL'`. | `components/InputSections.tsx` |
| 2026-02-24 | Hideout Mode toggle in Settings | Added `hideoutModeEnabled: boolean` to `UserPreferences`. Added "App Mode" section in Settings with an amber toggle switch. When OFF (default), app starts on NORMAL and the version selector is hidden from the nav. When ON, the NORMAL/HIDEOUT selector appears in the nav bar. Syncs on login and on settings save. | `components/UserSettings.tsx`, `utils/userPreferences.ts`, `App-refactored.tsx`, `components/AppHeader.tsx` |
...
| 2026-03-23 | Auto-Save Draft | Added debounced auto-save (1s delay) that persists `inputs`, `lenders`, and `appVersion` to **Supabase `drafts` table** (one draft per user, upsert) with localStorage fallback for local-auth users. Draft auto-restores on page load or user login. Draft cleared on explicit Save Deal or New Deal. Shows "Draft restored" notification on load. Visual auto-save indicator in header: "Saving…" (amber pulse) while saving, "Saved 12:34 PM" (green then gray) after success, "Save failed" (red) on error. New `drafts` table with RLS policies added to schema.sql. New `saveDraft()`, `loadDraft()`, `deleteDraft()` functions in `lib/database.ts`. | `App-refactored.tsx`, `components/AppHeader.tsx`, `lib/database.ts`, `supabase/schema.sql` |
| 2026-04-19 | CRITICAL: HIDEOUT Auto-Populate Defaults + Correct File Fix | **All previous April 18-19 commits edited `App.tsx` (DEAD CODE). The app loads from `App-refactored.tsx` via `index.tsx`.** Applied all fixes to the correct file. Added `HIDEOUT_DEFAULTS` constant in `App-refactored.tsx` with typical Walker & Walker / Hideout fees from HUD #6614838: walkerAttorneyFee=$750, walkerNotaryFee=$140, walkerSettlementFee=$1,583.40, walkerDocPrep=$175, walkerOvernight=$200, walkerWire=$200, titleSearchFee=$180, lendersTitleInsurance=$365.50, ownersTitlePolicy=$35, numberOfEndorsements=4, capitalImprovementFee=$50, resaleCertificateFee=$250, hideoutTransferFee=$2,160, surveyFee=$750, domesticLienSearch=$5, patriotActSearch=$5, recordingFees=$225, hideoutAnnualFee=$2,400, roamingwoodAnnual=$500, schoolTaxAnnual=$1,100, sewerWaterAnnual=$1,700. **Five integration points**: (1) `handleVersionChange` — applies defaults for any field still at 0 when switching to HIDEOUT; (2) `handleNewDeal` — pre-fills HIDEOUT defaults when creating new deal in HIDEOUT mode; (3) `handleLoadDeal` — detects old deals missing Walker/Hideout fields and fills them; (4) Supabase draft restore — syncs `appVersion` into `inputs` + applies defaults; (5) localStorage draft restore — same sync + defaults. Also fixed: Legal & Settlement zeroed in HIDEOUT mode (double-count with Walker fees), Title Ins. Rate hidden in HIDEOUT, individual Walker fee breakout on right panel (1101/1102/1107/1310), Legal & Settlement display hidden in HIDEOUT. Build: 764 modules clean. | `App-refactored.tsx`, `types.ts`, `utils/calculations.ts`, `components/InputSections.tsx`, `components/LoanEstimateCard.tsx` |
| 2026-04-17v3 | HIDEOUT Title Insurance Bug Fix + Walker Warning | **Bug fix**: In HIDEOUT mode, `calculateLoan()` was falling back to PA Title Insurance Rate Table when `lendersTitleInsurance` was $0 — producing $1,294.61 (sale/owner rate) instead of $0. The PA rate table has SALE rates, not lender rates. Walker/Penn Attorneys price lender's policies separately. Fixed: added `appVersion !== 'HIDEOUT'` guard on the rate table fallback — now in HIDEOUT mode, title insurance is $0 until the user enters the actual lender's policy amount from their HUD (line 1104). Also added amber warning notice in the Walker & Walker section when all Walker fees + Lender's Title Policy + Hideout Transfer are $0, prompting user to enter HUD values. Build: 764 modules clean. | `utils/calculations.ts`, `components/InputSections.tsx` |
| 2026-04-17v2 | HIDEOUT HUD Item Visibility — Section Headers, HUD Line Numbers, Always-Show | User couldn't find Walker/Hideout fields or see HUD items 1104-1107 on the right panel. Root cause: (1) Walker Charges section had NO visible header — inputs blended with surrounding fields; (2) FeeBreakdownItem hid all $0 items — key HIDEOUT HUD line items invisible until values entered; (3) Input/display labels lacked HUD line numbers making them impossible to match to actual HUD. **Fixes**: (a) Added purple section headers "Walker & Walker Title Charges (HUD 1100s)" and "Hideout POA Transfer & Prorations" in InputSections; (b) Added HUD line numbers to input labels (1101 Walker Attorney, 1102 Walker Notary, 1103 Title Search, 1104 Lender's Title Policy, 1105 CPL, 1106 Endorsements, 1107 Walker Settlement, 1301 Owner's Title Policy, 1304 Hideout Transfer, 1305 Capital Improvement, 1308 Resale Certificate, 1310 Doc/Overnight/Wire); (c) Added `alwaysShow` prop to FeeBreakdownItem — when true, item renders even at $0; (d) In HIDEOUT mode, all key HUD items display with HUD line numbers and `alwaysShow` in both Third Party Fees and Cash Required sections of LoanEstimateCard; (e) Fixed endorsement help text from "$100 each" to "$50 each". Build: 764 modules clean. | `components/FeeBreakdownItem.tsx`, `components/InputSections.tsx`, `components/LoanEstimateCard.tsx` |
| 2026-04-17 | HIDEOUT Closing Cost Fixes — Sell-Side Fees & HUD Alignment | Compared HUD-1 (File #6614838, 1355 Woodhill Lane) to HIDEOUT calculator. Found 3 issues: **(1) Sell-side missing Hideout Transfer & Legal fees** — Disposition costs only included commissions + transfer tax. Added `sellingLegalFees` ($) and `sellingHideoutTransferFee` ($, HIDEOUT only) fields to LoanInputs/CalculatedResults. These now flow into `totalExitCosts`, affect net profit, ROI, IRR, and all sensitivity scenarios. New "Selling Closing Costs" subsection in Exit Strategy UI. CostsBreakdown disposition section now shows Legal/Settlement and Hideout Transfer (sell) rows. ClosingProfitCard and ProfitTable also display when > $0. **(2) CostsBreakdown referencing non-existent result fields** — `totalSellingCommissionCost` and `sellingTransferTaxCost` were used in CostsBreakdown but never returned from calculateLoan(), so disposition costs always showed $0. Now properly computed and returned. **(3) Survey Fee, Domestic/Patriot searches hidden in HIDEOUT** — These HUD line items (1309=$750, 1302=$10) had input fields guarded by `appVersion !== 'HIDEOUT'`. Moved Survey Fee, Domestic Lien Search, and Patriot Act Search into a new "Survey & Search Fees" subsection visible in both modes. Mechanics Lien Ins. remains NORMAL-only. | `types.ts`, `utils/calculations.ts`, `components/InputSections.tsx`, `components/CostsBreakdown.tsx`, `components/ClosingProfitCard.tsx`, `components/ProfitTable.tsx` |
