...
| 2026-03-23 | Auto-Save Draft | Added debounced auto-save (1s delay) that persists `inputs`, `lenders`, and `appVersion` to **Supabase `drafts` table** (one draft per user, upsert) with localStorage fallback for local-auth users. Draft auto-restores on page load or user login. Draft cleared on explicit Save Deal or New Deal. Shows "Draft restored" notification on load. New `drafts` table with RLS policies added to schema.sql. New `saveDraft()`, `loadDraft()`, `deleteDraft()` functions in `lib/database.ts`. | `App.tsx`, `lib/database.ts`, `supabase/schema.sql` |
