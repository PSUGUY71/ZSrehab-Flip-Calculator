    ...
    <div className="space-y-4">
      ...
      {results.purchaseSensitivityScenarios && results.purchaseSensitivityScenarios.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg border-2 border-red-200 overflow-hidden">
          <div className="bg-red-100 p-2 text-xs font-bold uppercase text-center text-red-900 border-b-2 border-red-300">
            ⚠️ Purchase Price Sensitivity (MOST CRITICAL)
          </div>
          <div className="bg-red-50 p-2 text-[10px] text-red-800 text-center border-b border-red-200">
          ...
          {results.purchaseSensitivityScenarios.map((s, idx) => (
            ...
            >
              <div className={s.isBelowThreshold ? 'font-bold text-red-900' : ''}>{s.label}</div>
              <div>{formatCurrency(s.purchasePrice)}</div>
              <div className={s.profit > 0 ? 'text-green-700 font-semibold' : 'text-red-600 font-semibold'}>
                {formatCurrency(s.profit)}
              </div>
              <div className={s.profitChange >= 0 ? 'text-green-600' : 'text-red-500 font-semibold'}>
                {s.profitChange >= 0 ? '+' : ''}{formatCurrency(s.profitChange)}
              </div>
              <div className={s.isBelowThreshold ? 'text-red-700 font-bold' : s.margin >= 15 ? 'text-green-700' : 'text-orange-600'}>
                {formatPercent(s.margin)}
                {s.isBelowThreshold && <span className="text-[9px] block text-red-600">⚠️ Below 15%</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {results.rehabSensitivityScenarios && results.rehabSensitivityScenarios.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg border-2 border-orange-200 overflow-hidden">
          <div className="bg-orange-100 p-2 text-xs font-bold uppercase text-center text-orange-900 border-b-2 border-orange-300">
            Rehab Cost Sensitivity
          </div>
          <div className="bg-orange-50 p-2 text-[10px] text-orange-800 text-center border-b border-orange-200">
