    ...
    <div className="mt-3 border border-gray-200 rounded-lg overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {isExpanded && (
            <div className={`text-xs font-semibold ${isMatching ? 'text-green-600' : 'text-red-600'}`}>
      ...
      {isExpanded && (
        <div className="p-4 bg-white space-y-3">
          {/* Summary Row */}
          <div className="bg-gray-50 border border-gray-200 rounded p-2 flex justify-between items-center text-sm">
            <span className="font-semibold text-gray-700">Line Items Total:</span>
            <span className={`font-bold ${isMatching ? 'text-green-600' : 'text-red-600'}`}>
              {formatCurrency(lineItemsTotal)}
            </span>
          </div>
          {!isMatching && (
            <div className={`text-xs p-2 rounded ${difference > 0 ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
              {difference > 0 
          ...
          <button
            onClick={onAddItem}
            className="w-full py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded transition-colors"
          >
            + Add Line Item
          </button>
        </div>
